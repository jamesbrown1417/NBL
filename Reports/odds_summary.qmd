...
# Read positions data
positions <- read_csv("../Data/supercoach-data.csv", show_col_types = FALSE) |>
    select(player_name, player_team, position = supercoach_position_1)

# Read DVP data
dvp_all <- read_rds("../Data/processed_stats/dvp_all.rds")

# Map market names to DVP stat names
market_to_stat <- c(
...
# Add positions to odds data
all_odds_with_pos <- all_odds |>
    left_join(positions, by = c("player_name", "player_team")) |>
    filter(!is.na(position)) |>
    mutate(stat = market_to_stat[market_name])

# Join with DVP data
all_odds_with_dvp <- all_odds_with_pos |>
    left_join(dvp_all |> select(opposition, position, stat, dvp_value = value),
              by = c("opposition_team" = "opposition", "position", "stat"))

# Filter for favourable overs: positive diff_over_last_10 AND positive DVP
favourable_overs <- all_odds_with_dvp |>
    filter(!is.na(over_price)) |>
    filter(diff_over_last_10 > 0) |>
    filter(dvp_value > 0) |>
    select(match, agency, player_name, position, market_name, line, over_price,
           diff_over_last_10, dvp_value) |>
    arrange(match, desc(diff_over_last_10))

# Filter for favourable unders: positive diff_under_last_10 AND negative DVP
favourable_unders <- all_odds_with_dvp |>
    filter(!is.na(under_price)) |>
    filter(diff_under_last_10 > 0) |>
    filter(dvp_value < 0) |>
    select(match, agency, player_name, position, market_name, line, under_price,
           diff_under_last_10, dvp_value) |>
    arrange(match, desc(diff_under_last_10))

# Combine overs and unders
favourable_combined <- bind_rows(
    favourable_overs |> mutate(side = "Over", price = over_price, diff_l10 = diff_over_last_10) |>
        select(match, agency, player_name, position, market_name, line, side, price, diff_l10, dvp_value),
    favourable_unders |> mutate(side = "Under", price = under_price, diff_l10 = diff_under_last_10) |>
        select(match, agency, player_name, position, market_name, line, side, price, diff_l10, dvp_value)
) |>
    arrange(match, player_name, market_name)

# Get a list where each match is the name of the filtered DF
filtered_list_favourable <- map(all_matches, filter_by_match, df = favourable_combined) |>
  set_names(all_matches) |>
    keep(~ nrow(.x) > 0) |>
    map(~ datatable(.x))

# Use qreport to make tabset
qreport::maketabs(filtered_list_favourable, wide = TRUE)
```
...
# Get best under prices across match, market, and line
best_unders <- all_odds |>
    filter(market_name != "Total Points") |> 
    filter(!is.na(under_price)) |>
    group_by(match, player_name, market_name, line) |>
    slice_max(under_price, n = 1, with_ties = FALSE) |>
    ungroup() |>
    # Join with positions data (already loaded above)
    left_join(positions, by = c("player_name", "player_team")) |>
    # Add stat column for DVP join
    mutate(stat = market_to_stat[market_name]) |>
    # Join with DVP data (already loaded above)
    left_join(dvp_all |> select(opposition, position, stat, dvp_value = value),
              by = c("opposition_team" = "opposition", "position", "stat")) |>
    mutate(dvp_value = round(dvp_value, 2)) |>
    select(match, agency, player_name, player_team, market_name, line, under_price, diff_under_last_10, dvp_value) |>
    arrange(player_name, desc(line))

# Get unique matches and markets
unique_matches <- best_unders |> distinct(match) |> pull(match)
unique_markets <- best_unders |> distinct(market_name) |> pull(market_name)

# Create nested list structure for qreport::maketabs
# Outer level: matches, Inner level: markets
nested_list <- list()

for (m in unique_matches) {
    match_data <- best_unders |> filter(match == m)

    market_list <- list()
    for (mkt in unique_markets) {
        market_data <- match_data |>
            filter(market_name == mkt) |>
            select(-match, -market_name)

        if (nrow(market_data) > 0) {
            market_list[[mkt]] <- datatable(market_data)
        }
    }

    if (length(market_list) > 0) {
        nested_list[[m]] <- market_list
    }
}

# Generate nested tabs
for (m in names(nested_list)) {
    cat(paste0("\n### ", m, "\n\n"))
    cat("::: {.panel-tabset}\n\n")

    for (mkt in names(nested_list[[m]])) {
        cat(paste0("#### ", mkt, "\n\n"))
        print(htmltools::tagList(nested_list[[m]][[mkt]]))
        cat("\n\n")
    }
