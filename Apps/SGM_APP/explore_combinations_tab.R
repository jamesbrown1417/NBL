...
##%######################################################%##
#                                                          #
####    1.) Explore redundant leg SGMs (same player)    ####
#                                                          #
##%######################################################%##
...
row_combinations <- combn(nrow(tab_sgm_bets_same_player), 2)
...
list_of_dataframes <-
  map(
    ...
    .f = function(i) {
        tab_sgm_bets_same_player[row_combinations[, i], ] |> 
        mutate(combination = i)
    }
  )

# Keep only those where the same player appears twice
retained_combinations <-
  list_of_dataframes |> 
  # Keep only dataframes where first and second row match are equal
  keep(~.x$match[1] == .x$match[2]) |> 
  # Keep only dataframes where first and second row player_name are equal
  keep(~.x$player_name[1] == .x$player_name[2]) |> 
  # Keep only dataframes where first and second row market_name are equal
